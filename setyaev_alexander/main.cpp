...
void prepare_temp_directory() {
  std::filesystem::create_directory(std::string(uni_course_cpp::config::kTempDirectoryPath));
}
...
int main() {
  ...
  for (int i = 0; i < graphs_count; i++) {
    ...
    logger.log(uni_course_cpp::printing::generation_finished_string(i, graph_description));

    const auto graph_json = uni_course_cpp::printing::json::graph_to_string(graph);
