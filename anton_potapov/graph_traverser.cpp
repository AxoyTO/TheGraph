...
std::map<VertexId, int> GraphTraverser::get_updated_depths(const Graph& graph,
                                                           int start_depth) {
  ...
  if (start_depth == INIT_DEPTH) {
    ...
    const auto vertices_at_max_correct_depth =
        graph.get_vertices_at_depth(max_correct_depth);
    ...
    if (max_correct_depth > 0) {
      const auto vertices_at_depth_before_max_correct =
          graph.get_vertices_at_depth(max_correct_depth - 1);
  ...
  while (!bfs_queue.empty()) {
    ...
    for (const auto& connected_edge_id :
         graph.get_vertex(current_vertex_id).connected_edges()) {
...
std::map<VertexId, int> GraphTraverser::get_depths(const Graph& graph) {}

int GraphTraverser::get_updated_vertex_depth(const Graph& Graph,
                                             VertexId vertex_id,
                                             int start_depth) {}

int GraphTraverser::get_vertex_depth(const Graph& Graph, VertexId vertex_id) {}
}  // namespace uni_cource_cpp
