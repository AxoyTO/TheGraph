...
class Vertex {
 public:
  ...
  Vertex(const VertexId& _id, const int _depth) : id(_id), depth(_depth) {}
...
class Edge {
 public:
  ...
  Edge(const EdgeId& _id,
       ...
       const Color& _color)
      : id(_id), vertex_id1(_vertex_id1), vertex_id2(_vertex_id2), color(_color) {}
...
class Graph {
 public:
  ...
  void add_edge(const VertexId& from_vertex_id, const VertexId& to_vertex_id, const Edge::Color& edge_color);

  bool has_vertex(const VertexId& id) const;

  bool have_vertices_in_depth(const int depth) const;
  ...
  void fill_vertices_in_depth(const int depth, const VertexId id);

  const std::vector<VertexId>& get_vertices_ids_in_depth(const int depth) const;

  const int get_depth() const { return vertices_in_depth_.size(); }
 ...
 private:
  ...
  std::vector<std::vector<VertexId>> vertices_in_depth_;
