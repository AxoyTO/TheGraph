...
void GraphPrinting::logStart(Logger& logger, int graphIndex) {
...
void GraphPrinting::logColors(Logger& logger, Graph graph) {
  const std::array<Edge::Color, 5> colors = {
      Edge::Color::Gray, Edge::Color::Green, Edge::Color::Blue,
      Edge::Color::Yellow, Edge::Color::Red};
  for (int i = 0; i < colors.size(); i++) {
    logger.log(uni_course_cpp::edgeColorToString(colors[i]) + ": " +
               std::to_string(graph.getNumEdgeByColor(colors[i])));
    if (i + 1 != colors.size())
      logger.log(", ");
  }
}
void GraphPrinting::logEnd(Logger& logger, Graph graph, int graphIndex) {
  logger.log(getTimeByString() + ": Graph " + std::to_string(graphIndex) +
             ", Generation Finished {  \n");
...
void GraphPrinting::writeGraphToFile(const Graph& graph, int index) {
