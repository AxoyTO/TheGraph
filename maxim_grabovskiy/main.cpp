...
class Graph {
 public:
  struct Edge {
    ...
    Edge(EdgeId _id, int _fromVertexId, int _toVertexId)
  ...
  void spawnVertex(VertexId parentId) {
    addVertex();
    addEdge(parentId, vertexIdCounter);
  }

 private:
  VertexId vertexIdCounter = 0;
  EdgeId edgeIdCounter = 0;
  VertexId getNewVertexId() { return vertexIdCounter++; }
  EdgeId getNewEdgeId() { return edgeIdCounter++; }
};
int main() {
  Graph graph;
  graph.addVertex();
