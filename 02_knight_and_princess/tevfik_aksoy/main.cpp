...
using DestID_EdgeID_SrcDepth_DestDepth_Color = std::tuple<VertexId, EdgeId, VertexDepth, VertexDepth, EdgeColor>;  // dest, edge_val, src_depth, dest_depth
...
public:
    ...
    Edge(VertexId _source, VertexId _destination, VertexId _id, EdgeColor _color = "")
    ...
    std::string to_json(int end_flag) const{
        std::string str = "\t{ \"id\": " + std::to_string(id) +
                          ...
                          std::to_string(destination) + R"(], "color": ")" +
                          color + "\"";
        if(!end_flag)
            str += " },\n";
        else
...
public:
    ...
    void insert_node(int c_node, int local_node_num, int local_edge_value, int src_depth, int dest_depth, const string &color);
...
void Graph::insert_node(int source, int destination, int edge_id, int source_depth, int destination_depth, const string &color){
    graph_data.resize(st_vertex_count * 2);
    graph_data[source].push_back(std::make_tuple(destination, edge_id, source_depth, destination_depth, color));
    if(color != "green"){
        graph_data[destination].push_back(std::make_tuple(source, edge_id, destination_depth, source_depth, color));
        vertices.push_back(Vertex(source, source_depth));
    }
    edges.push_back(Edge(source, destination, edge_id, color));
}

void Graph::insert_edge(Vertex src, Vertex dest, int edge_val, const string &color){
    if(!there_is_edge(Edge(src.id, dest.id, edge_val)))  // if there is an edge connecting these vertices already, no need to duplicate
    {
        if(src.id == dest.id){
            if(!there_is_vertex(src.id))  // a green edge for a node that has just become a source(and a destination itself)
            {
                st_vertex_count++;
...
void Graph::generate_green_edges(){
    ...
    for(int i = 0; i < total_num_nodes(); i++){
        ...
        if(probability_ >= condition_){
            insert_edge(Vertex(i, depth_of(i)), Vertex(i, depth_of(i)), st_edge_val, "green");
        }
    }
...
void Graph::generate_blue_edges(){
    ...
    for(int i = 0; i < total_num_nodes(); i++){
        if(depth_of(i) == depth_of(i + 1)){
            probability_ = rand() % 100;
            if(probability_ > condition_){
                insert_edge(Vertex(i, depth_of(i)), Vertex(i + 1, depth_of(i)), st_edge_val, "blue");
            }
        }
    }
}
    ...
    for(int i = total_num_nodes() - 1; i > 0; i--){
        ...
        for(int j = i - 1; j > 0; j--){
            if(depth_of(j) == depth_of(i) - 1){
                probability_ = rand() % 100;
                if(probability_ > condition_){
                    insert_edge(Vertex(j, depth_of(j)), Vertex(i, depth_of(i)), st_edge_val, "yellow");
                }
    ...
    for(int i = 0; i < max_depth; i++){
        while(m != nodes_with_depth(i).size()){
            while(n != nodes_with_depth(i + 2).size()){
                probability_ = rand() % 100;
                if(probability_ > condition_)
                    insert_edge(nodes_with_depth(i).at(m), nodes_with_depth(i + 2).at(n++), st_edge_val, "red");
            }
            n = 0;
            m++;
        }
...
bool Graph::there_is_edge(const Edge &edge){
    ...
    for(int i = 0; i < graph_data.size(); i++){
        ...
        for(auto &j: graph_data[i]){
            ...
            if((src_node_id == edge.source) &&
               (dest_node_id == edge.destination)){
                return true;
            }
        }
    }
    ...
    for(int i = 0; i < graph_data.size(); i++){
        for(int j = 0; j < graph_data[i].size(); j++){
            cout << "(" << i << ", " << get<0>(graph_data[i][j]) << ", "
                 << get<1>(graph.graph_data[i][j]) << ", " << get<2>(graph.graph_data[i][j])
