...
static int st_node_count = 0; //st_node_count is the amount of vertices in the graph.
static int st_edge_val = 0;   //st_edge_val is also the amount of edges in the graph.
...
//Tuple of <int,int,int,int> for realization of vector of vectors of <int,int,int,int>
typedef std::tuple<int, int, int, int, string> DestID_EdgeID_SrcDepth_DestDepth; //dest, edge_val, src_depth, dest_depth
...
public:
    ...
    std::string to_json(vector<int> edge_ids, bool end_flag) const{
        std::string str = "\t{ \"id\": " + std::to_string(id_) + ", \"edge_ids\": [";
...
public:
    ...
    std::string to_json(int end_flag) const{
        std::string str =
                "\t{ \"id\": " + std::to_string(id_) + ", \"vertex_ids\": [" + std::to_string(src_node.GetID()) + ", " +
                std::to_string(dest_node.GetID()) + R"(], "color": ")" + color_ + "\"";
        if(!end_flag)
            str += " },\n";
        else
            str += " }\n  ]\n}\n";
...
public:
    ...
    vector<vector<DestID_EdgeID_SrcDepth_DestDepth>> GetGraphVec() const{ return graphVec; }

    //insert_node adds a new node and inserts a new simple(gray) edge to connect the new node with the source
    void insert_node(int c_node, int local_node_num, int local_edge_value, int src_depth, int dest_depth,
                     const string &color);
    ...
    //Function that generates(10%) green edges. A green edge is an edge from a vertex which is source and destination itself
    void generate_green_edges();

    //Function that generates(25%) blue edges. A blue edge is an edge which connects two neighbor vertices in the same depth
    void generate_blue_edges();

    //Function that generates yellow edges. A yellow edge is an edge which connects a vertex with a deeper(by 1 level) vertex on variable probability
    void generate_yellow_edges();

    //Function that generates red edges. A red edge is an edge which connects a vertex with a deeper(by 2 levels) vertex on 33% probability
    void generate_red_edges();

    //Checks if there is such an edge already
    bool there_is_edge(const Edge &edge);

    //Checks if there is such vertex as a source, if there is not, then new_node_flag != 0
    bool there_is_vertex(int src_node_id);

    //Checks if there is such vertex as a destination, if there is not, then dest_node_flag !=0;
...
private:
    int probability_ = 0;
    int condition_ = 0;
    vector<std::tuple<int, int, string>> edgeSrcDestColor; //vector of pairs to keep data of source and destination vertices of an edge
    vector<vector<DestID_EdgeID_SrcDepth_DestDepth>> graphVec;
};

void Graph::insert_node(int c_node, int local_node_num, int local_edge_value, int src_depth, int dest_depth,
                        const string &color){
    graphVec.resize(st_node_count * 2);
    graphVec[creator_node].push_back(std::make_tuple(local_node_num, local_edge_value, src_depth, dest_depth, color));
    graphVec[local_node_num].push_back(std::make_tuple(c_node, local_edge_value, dest_depth, src_depth, color));
    edgeSrcDestColor.emplace_back(c_node, local_node_num, color);
}

void Graph::insert_edge(Node src, Node dest, int edge_val, const string &color){

    Edge e(src, dest, edge_val);
    if(!there_is_edge(e)) //if there is an edge connecting these vertices already, no need to duplicate
    {

        if(src.GetID() == dest.GetID()){
            if(!there_is_vertex(
                    src.GetID())) //a green edge for a node that has just become a source(and a destination itself)
            {
                st_node_count++;
                insert_node(src.GetID(), dest.GetID(), st_edge_val++, src.GetDepth(), dest.GetDepth(), color);
...
void Graph::generate_vertices(){
    //int successful_attempts = 0; //successful attempts count how many nodes are successfully created in current depth
    Node src, dest;
    if(st_condition <= 100){
...
void Graph::generate_blue_edges(){
    condition_ = 75; //Синяя: 25% что соседние вершины, находящиеся на одной глубине, будут соединены.
        ...
        if(graphVec[i].empty())
            ...
            for(int j = graphVec[i].size() - 1; j >= 0; j--){
                ...
                if(probability_ >= condition_){
                    ...
                    if((j == graphVec[i].size() - 1) && (i != graphVec.size() - 1) && (!graphVec[i + 1].empty())){
                    ...
                    if(probability_ >= condition_){
                        ...
                        for(size_t j = 0; j < graphVec[i].size(); j++){
                            if(j < graphVec[i].size())
                                if((get<3>(graphVec[i][j]) == get<3>(graphVec[i + 1][0])) &&
                                   (get<0>(graphVec[i][j]) == get<0>(graphVec[i + 1][0]) - 1)){
                                    src.SetID(get<0>(graphVec[i][j]));
                                    src.SetDepth(get<3>(graphVec[i][j]));
...
void Graph::generate_red_edges(){
    condition_ = 67; //Красная: 33% что вершина будет соединена с рандомной вершиной, находящейся на 2 уровня глубже.
...
bool Graph::there_is_edge(const Edge &edge){
    ...
    for(int i = 0; i < graphVec.size(); i++){
        ...
        for(auto &j: graphVec[i]){
            ...
            if((src_node_id == edge.src_node.GetID()) && (dest_node_id == edge.dest_node.GetID())){
                return true;
            }
        }
    }
...
[[maybe_unused]] void Graph::displayGraph(){
    for(int i = 0; i < graphVec.size(); i++){
        for(int j = 0; j < graphVec[i].size(); j++){
            cout << "(" << i << ", " << get<0>(graphVec[i][j]) << ", " << get<1>(GetGraphVec()[i][j])
                 << ", " << get<2>(GetGraphVec()[i][j]) << ", " << get<3>(GetGraphVec()[i][j]) << ")";
        }
        if(!graphVec[i].empty())
            cout << endl;
    }
...
struct Logger{
    ...
    void start(){
        ...
        time_str = ctime(&now);
        time_str.pop_back();
        PrintandWrite(cout, logfile, time_str + ": Graph " + std::to_string(k) + ", Generation Started\n");
    }
    ...
    void end(const Graph &graph){
        ...
        time_str = ctime(&now);
        time_str.pop_back();
        PrintandWrite(cout, logfile, time_str + ": Graph " + std::to_string(k++) + ", Generation Ended {\n");
        PrintandWrite(cout, logfile, "  depth: " + std::to_string(max_depth) + ",\n");
        PrintandWrite(cout, logfile, "  new_vertices_num: " + std::to_string(new_vertices_num) + ",\n");
        PrintandWrite(cout, logfile, "  vertices: " + std::to_string(graph.total_num_nodes()) + ", [");

        for(int i = 0; i <= max_depth; i++){
            PrintandWrite(cout, logfile,
                          "depth_" + std::to_string(i) + "_" + std::to_string(graph.vertices_in_depth(i)));
            if(i != max_depth)
                PrintandWrite(cout, logfile, ", ");
        }
        PrintandWrite(cout, logfile, "],\n");
        PrintandWrite(cout, logfile, " edges: " + std::to_string(graph.total_num_edges()) + ", {");
        for(int i = 0; i < colors.size(); i++){
            PrintandWrite(cout, logfile, colors[i].first + ": " + std::to_string(colors[i].second));
            if(i + 1 != colors.size())
                PrintandWrite(cout, logfile, ", ");
        }
        PrintandWrite(cout, logfile, "}\n}\n");
