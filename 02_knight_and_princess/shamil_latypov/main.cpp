...
struct Edge {
    ...
    void to_string() {
        out_json << "{ \"id\": " << id << ", \"vertex_ids\": [" << v1->id << ", " << v2->id << "] }";
    }    
};

void get_graph(Edge *edge_mas, Vertex *vert_mas, int numedge, int numvertex)
{
    std::pair <int, int> graph_data[numedge] = {{0, 1}, {0, 2}, {0, 3}, {1, 4}, {1, 5}, {1, 6},
...
int main()
{
    ...
    for (int i = 0; i < numedge; i++) edge_mas[i].id = i;
    for (int i = 0; i < numvertex; i++) vert_mas[i].id = i;
